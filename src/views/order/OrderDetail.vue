<template>
    <div id="order-detail">
        <NavBar class="nav-bar">
            <template v-slot:default> 订单详情（未完善） </template>
        </NavBar>
    ...
    <div class="order-info">
      ...
      <!-- orderInfo.statusCode == 1为新订单，显示支付按钮 。等于2时为已支付订单，显示确认按钮 -->
      <van-button
...
import {getOrderDetail,payOrder,payOrderStatus } from '@/network/order';
...
export default defineComponent({
    ...
    setup(){
        ...
        getOrderDetail($route.query.id).then((res) => {
            ...
            orderDetail.orderInfo.address ='临时地址'
            //订单状态
            let statusAll = [
            "",
            "已下单",
